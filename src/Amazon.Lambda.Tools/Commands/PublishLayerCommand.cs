...
namespace Amazon.Lambda.Tools.Commands
{
    public class PublishLayerCommand : LambdaBaseCommand
    {
        ...
        private async Task<CreateLayerZipFileResult> CreateRuntimePackageStoreLayerZipFile(string layerName, string s3Prefix)
        {
            ...
            // "dotnet store" writes the store using the .NET architecture directory name (e.g. "x64"), which differs from the
            // AWS Lambda architecture value (e.g. "x86_64"), so translate before building the lookup path.
            var storeArchitecture = this.GetStringValueOrDefault(this.Architecture, LambdaDefinedCommandOptions.ARGUMENT_FUNCTION_ARCHITECTURE, false);
            var architectureDirectory = LambdaUtilities.DetermineStoreArchitectureDirectory(storeArchitecture);

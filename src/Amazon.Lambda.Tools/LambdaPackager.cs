...
namespace Amazon.Lambda.Tools
{
    ...
    public static class LambdaPackager
    {
        ...
        {
            var includedFiles = GetFilesToIncludeInArchive(sourceDirectory, flattenRuntime);
            BundleFiles(zipArchivePath, sourceDirectory, includedFiles, logger);
        }
        ...
        public static void BundleFiles(string zipArchivePath, string rootDirectory, string[] files, IToolLogger logger)
        {
            ...
            BundleFiles(zipArchivePath, rootDirectory, includedFiles, logger);
        }

        /// <summary>
        /// Bundle the given files into a deployment zip archive using the built-in .NET compression
        /// libraries. The archive entries are marked with Unix file permissions so files such as the
        /// custom runtime "bootstrap" retain their execute bit when extracted on Lambda. This replaces
        /// the previous approach that shelled out to the native "zip" utility on Linux/macOS and to the
        /// vendored build-lambda-zip.exe utility on Windows.
        /// </summary>
        ...
        private static void BundleFiles(string zipArchivePath, string rootDirectory, IDictionary<string, string> includedFiles, IToolLogger logger)
        {
            EnsureBootstrapLinuxLineEndings(rootDirectory, includedFiles);

            ManagedZipArchive.BundleFiles(zipArchivePath, rootDirectory, includedFiles, logger);
        }
